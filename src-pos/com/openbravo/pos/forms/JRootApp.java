...
import com.openbravo.pos.smj.ERPListener;
...
public class JRootApp extends JPanel implements AppView {
    ...
    private ERPListener eRPListener = null;
    ...
    public static   String jmsOutgoingQueue = "";
    public static   String jmsInQueue = "";
    public static   String jmsUrlIn = "";
    public static   String jmsStoreFolder = "";
    
    static {        
        initOldClasses();
    }
    public static   String jmsUserLogin;
    ...
    public boolean initApp(AppProperties props) {
        ...
        BufferedImage put = m_dlSystem.getResourceAsImage("mainImage");
        if(put !=  null)
            jLabel3.setIcon(new ImageIcon(put));
        
        
        // SMJ   - set JMS locations for ERP synchronization
        
        jmsUrl = m_dlSystem.getResourceAsText("jms.url.out"); 
        jmsUrlIn = m_dlSystem.getResourceAsText("jms.url"); 
        jmsStoreFolder = m_dlSystem.getResourceAsText("jms.store.folder"); 
        jmsOutgoingQueue = m_dlSystem.getResourceAsText("jms.outqueue"); 
        jmsUserLogin = m_dlSystem.getResourceAsText("jms.userLogin"); 
        String password = m_dlSystem.getResourceAsText("jms.password"); 
        jmsInQueue = m_dlSystem.getResourceAsText("jms.inqueue"); 
        
        if(password.indexOf("crypt") == 0){
            AltEncrypter cypher = new AltEncrypter("cypherkey" + jmsUserLogin);
            jmsPassword = cypher.decrypt(password.substring(6));
        }else
            jmsPassword = password;
        
        System.out.println("URL del server JMS:"+jmsUrl);
        System.out.println("URL de la cola de salida JMS:"+jmsOutgoingQueue);
        ...
        //Creacion de la cola enbebida
        startActiveMQ();
        
        //Conexion con la cola
...
/********************************Presonalizacion************************************/
    /**
     * Inicializa el sevidor activeMQ - Inicialize ActiveMQ server
     */
    private void startActiveMQ(){
        SimpleAuthenticationPlugin              authentication = null;
        java.util.List<AuthorizationEntry>      authenticationEntrys = null;
        AuthorizationPlugin                     authorization = null;
        AuthorizationEntry                      authorizationEntry = null;
        AuthorizationMap                        authorizationMap = null;
        BrokerService                           broker = null;
        DocumentBuilder                         builder  =  null;
        Document                                document = null;
        NodeList                                eleChildren = null;
        Element                                 eleChild = null;
        Element                                 eleRoot = null;
        org.xml.sax.InputSource                 inStream = null;
        NetworkConnector                        networkConnector = null;
        java.util.List<NetworkConnector>        networkConnectors = null;
        KahaPersistenceAdapter                  persistenceAdapter = null;
        java.util.List<AuthenticationUser>      users = null;
        String                                  xml;
        int                                     xmlIndex = 0;

        try {
            //load XML from configuration
            xml = m_dlSystem.getResourceAsText("activeMQConfiguration");
            xml = xml.trim();
            xmlIndex = xml.indexOf("<entityDetail>");
            xml = xml.substring(xmlIndex);
            ...
            //creat broker
            eleRoot = (Element)document.getElementsByTagName("broker").item(0);
            broker = new BrokerService();
            broker.setUseJmx(true);
                        
            
            //create networkConnectors 
            //class responsable for store and forward
System.out.println("static:("+jmsUrlIn+")");
//            networkConnector = new NetworkConnector(URI.create(jmsUrl)) {};           
//            networkConnector = broker.addNetworkConnector("static:(failover:"+jmsUrlIn+")");
            networkConnector = broker.addNetworkConnector("static:("+jmsUrlIn+")");
            networkConnector.setUserName(jmsUserLogin);
            networkConnector.setPassword(jmsPassword);
            
            //create  persistenceAdapter           
            persistenceAdapter = new KahaPersistenceAdapter();
            persistenceAdapter.setDirectory(new File(jmsStoreFolder)); 
            persistenceAdapter.setMaxDataFileLength(20 * 1024 * 1024);  
            broker.setPersistenceAdapter(persistenceAdapter);
            
            //create user list
            authentication = new SimpleAuthenticationPlugin();
            users = new ArrayList<AuthenticationUser>();
            
            eleChildren = document.getElementsByTagName("user");
            for(xmlIndex =0; xmlIndex <eleChildren.getLength(); xmlIndex++){
                eleChild = (Element) eleChildren.item(xmlIndex);
                users.add(new AuthenticationUser(eleChild.getAttribute("login"),eleChild.getAttribute("password"),eleChild.getAttribute("roles")));
            }
            ...
            authenticationEntrys = new ArrayList<AuthorizationEntry>();
            
            
            eleChildren = document.getElementsByTagName("queue");
            for(xmlIndex =0; xmlIndex <eleChildren.getLength(); xmlIndex++){
                eleChild = (Element) eleChildren.item(xmlIndex);
                authorizationEntry = new AuthorizationEntry();
                authorizationEntry.setQueue(eleChild.getAttribute("name"));
                authorizationEntry.setRead(eleChild.getAttribute("read"));
                authorizationEntry.setWrite(eleChild.getAttribute("write"));
                authorizationEntry.setAdmin(eleChild.getAttribute("admin"));
                authenticationEntrys.add(authorizationEntry);
            }
            
            eleChildren = document.getElementsByTagName("topic");
            for(xmlIndex =0; xmlIndex <eleChildren.getLength(); xmlIndex++){
                eleChild = (Element) eleChildren.item(xmlIndex);
                authorizationEntry = new AuthorizationEntry();
                authorizationEntry.setTopic(eleChild.getAttribute("name"));
                authorizationEntry.setRead(eleChild.getAttribute("read"));
                authorizationEntry.setWrite(eleChild.getAttribute("write"));
                authorizationEntry.setAdmin(eleChild.getAttribute("admin"));
                authenticationEntrys.add(authorizationEntry);
...
//            
//            authorizationEntry = new AuthorizationEntry();
//            authorizationEntry.setTopic("ActiveMQ.Agent.>");
//            authorizationEntry.setRead(eleChild.getAttribute("read"));
//            authorizationEntry.setWrite(eleChild.getAttribute("write"));
//            authorizationEntry.setAdmin(eleChild.getAttribute("admin"));
//            authenticationEntrys.add(authorizationEntry);
            ...
            authorizationMap = new DefaultAuthorizationMap(authenticationEntrys);
            authorization = new AuthorizationPlugin(authorizationMap);          
             
            //configure broker
            broker.setPlugins(new BrokerPlugin[]{authentication,authorization});
//            broker.addConnector(eleRoot.getAttribute("connector"));
            //broker.addConnector("tcp://localhost:61617");
            //url Local for store and forward
    ...
    /**
     * Crea la conexion a la cola para enviar mensajes al ERP
     */
    private void startTopicConnection(){
        eRPListener = new ERPListener(m_props,m_dlSystem, getProperties().getProperty("machine.hostname"));
       // try {
            eRPListener.start();
...
/**********************FIN de la presonalizacion*****************************************/        
    ...
    private void initComponents() {
        ...
        jLabel1.setText("<html><center><br>" +
            "Developed by <A href=\"http://www.smartjsp.com\">http://www.smartjsp.com</A>(c) 2012<br> " +
            "100% Open Source with GNU (GPL V3) License<br>" +
            "Powered by OpenBravo POS<br>" +
            "</center>");
