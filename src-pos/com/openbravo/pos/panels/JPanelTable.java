...
public abstract class JPanelTable extends JPanel implements JPanelView, BeanFactoryApp {
    ...
    protected void startNavigation() {
        ...
        if (bd == null) {
            ...
            c.applyComponentOrientation(getComponentOrientation());
            toolbar.add(c);
        }if(this instanceof PeoplePanel){
            if(!app.getAppUserView().getUser().hasPermission("com.openbravo.pos.smj.SyncPanel")){
                ListProviderCreator dataprov = (ListProviderCreator)bd.getDataprov();
                TableDefinition table = dataprov.getTable();
                table.setWhere(" where id != '0'");
                dataprov.setTable(table); 
                bd.setDataprov(dataprov);
            }else{
                ListProviderCreator dataprov = (ListProviderCreator)bd.getDataprov();
                TableDefinition table = dataprov.getTable();
                table.setWhere("");
