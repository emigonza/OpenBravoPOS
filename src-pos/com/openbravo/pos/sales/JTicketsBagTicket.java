...
public class JTicketsBagTicket extends JTicketsBag {
    ...
    private static Logger logger = Logger.getLogger(JTicketsBagTicket.class.getName());
    private TicketInfo m_ticketCopyTovoid;
    ...
    public void voidTicket() {
        if (m_ticketCopyTovoid != null) {           
            // Para editar borramos el ticket anterior
            try {   
                m_dlSales.voidTicket(m_ticketCopyTovoid, m_App.getInventoryLocation());
                sendTicketVoided();
            } catch (BasicException eData) {
                MessageInf msg = new MessageInf(MessageInf.SGN_NOTICE, AppLocal.getIntString("message.nosaveticket"), eData);
                msg.show(this);                
    ...
    /**
     * Modificacion para control de devoluciones y anulaciones. Correccion de bug para visualizacion de tickets para imprimir - 
     * Change to control returns and cancellations. Fix bug for visualization of tickets to print
     */
    private void printTicket() {
        ...
        if (m_ticket == null) {
            ...
            try {
                ...
                script.put("infoDV", m_dlSystem.getResourceAsText("info.DV"));
                script.put("infoAddress", m_dlSystem.getResourceAsText("info.address"));
                script.put("infoCity", m_dlSystem.getResourceAsText("info.city"));
    ...
    /******************************************************/
          public TicketInfo getTicketCopy() {
            return m_ticketCopy;
          }
          public TicketInfo getTicketCopyTovoid() {
            return m_ticketCopyTovoid;
          }
    /******************************************************/
    ...
    private void m_jRefundActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_m_jRefundActionPerformed
       ...
       for(int i = 0; i < m_ticket.getLinesCount(); i++) {
            try {
                ...
                Double refund = dlSales.getTicketLineRefund(newline.getTicketLineOld(), newline.getTicketIdOld());
                if(refund != 0){
                    canDelete = false;
                }
                
                refund += newline.getMultiply();
                if(refund > 0){
                    newline.setMultiply(refund);
        ...
        m_ticketCopy = null;
//        m_ticketCopy = m_ticket; 
        if(m_ticket.getTotal() == 0)
            canDelete = false;
        m_ticketCopyTovoid = m_ticket; 
        m_TicketsBagTicketBag.showRefund(canDelete);
        ...
        refundticket.setRefundticketId(m_ticket.getTicketId());  
...
private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
    ...
    private void sendTicketVoided(){
        try {
            String jmsUrl = m_dlSystem.getResourceAsText("jms.url.out"); 
            String jmsOutgoingQueue = m_dlSystem.getResourceAsText("jms.outqueue"); 
            String jmsInQueue = m_dlSystem.getResourceAsText("jms.inqueue"); 
            String jmsUserLogin = m_dlSystem.getResourceAsText("jms.userLogin"); 
            String jmsPassword = m_dlSystem.getResourceAsText("jms.password"); 
            if(jmsPassword.indexOf("crypt") == 0){
                AltEncrypter cypher = new AltEncrypter("cypherkey" + jmsUserLogin);
                jmsPassword = cypher.decrypt(jmsPassword.substring(6));
            }
            
            String xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>";
            xml += "<entityDetail>";
            xml += "<type>VOID-TICKET</type>";
            xml += "<id>"+m_ticketCopyTovoid.getTicketId()+"</id>";
            ...
            String subject = jmsOutgoingQueue;
            String url = jmsUrl;

            ConnectionFactory connectionFactory = new ActiveMQConnectionFactory(jmsUserLogin ,jmsPassword,url);
