...
public JRefundLines(DataLogicSystem dlSystem, JPanelTicketEdits jTicketEdit) {
        ...
        jTicketEdit.setjRefundLines(this);
    }
    ...
    /**
     * Modificacion para controlar la cantidad de productos devueltos en una linea de un ticket - 
     * Change to control the amount of returned products in a line for a ticket
     * @param line 
     */
    public void returnLines(TicketLineInfo line){
        for (int i = 0; i < m_aLines.size(); i++) {
            TicketLineInfo oLine = (TicketLineInfo) m_aLines.get(i);
            if(oLine.getProductID().equalsIgnoreCase(line.getProductID())  && oLine.getPrice() == line.getPrice()){
                oLine.setMultiply(oLine.getMultiply() +  (line.getMultiply() * -1));
                break;
            }
        }
