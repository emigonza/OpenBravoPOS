...
//        spr = createSaveProvider(table);

        editor = new AttributeUseEditor(app, dirty);
    }
    
    /**
     * crea un saveProvider vacio para bloquear los botones de guardar, 
     * nuevo y borrar
     * @return SaveProvider vacio
     * @autor Carlos PrietocreateSaveProvider
     */
    private SaveProvider createSaveProvider(Table table){
        ...
        if(app.getAppUserView().getUser().hasPermission("com.openbravo.pos.inventory.ProductsPanel"))
            return new SaveProvider(null,null,null);
             ...
             return row.getSaveProvider(app.getSession(), table);
