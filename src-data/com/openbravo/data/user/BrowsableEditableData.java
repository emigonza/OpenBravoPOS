...
//    private DocumentLoader m_keyvalue;
    ...
    private static Logger logger = Logger.getLogger(BrowsableEditableData.class.getName());
...
//        m_keyvalue = DocumentLoaderBasic.INSTANCE;
        ...
        if (m_Dirty.isDirty()) {
            if (m_iState == ST_UPDATE) {
                if(canSave()){
                    ...
                    JOptionPane.showMessageDialog(null, LocalRes.getIntString("message.cannotsavetaxid"));
                }
                
            } else if (m_iState == ST_INSERT) {
                if(canSave()){
                    if(m_editorrecord instanceof  CustomersView){
                        CustomersView cv = ((CustomersView)m_editorrecord);
                        if(cv.getSearchkey().getText().trim().equals("")){
                            Date d = new Date();
                            cv.getSearchkey().setText("TEMP"+d.getTime());
                        }
                    }
    ...
    private boolean canSave(){
        if(m_editorrecord instanceof  CustomersView){
            CustomersView cw = (CustomersView)m_editorrecord;
            return cw.validateTaxId();
        }else{
            return true;
        }
    ...
    private void sendCustomerToQueue(){
        if(m_editorrecord instanceof  CustomersView){
            try {
                CustomersView cv = ((CustomersView)m_editorrecord);
                cv.setOrganization(JRootApp.jmsInQueue);  
                String xml = cv.getXML();
                logger.log(Level.SEVERE,"\n+++++++++++++++++++++++++\n\n");
                logger.log(Level.SEVERE, xml);
                String subject = JRootApp.jmsOutgoingQueue;
                String url = JRootApp.jmsUrl;
                
                ConnectionFactory connectionFactory = new ActiveMQConnectionFactory(JRootApp.jmsUserLogin ,JRootApp.jmsPassword,url);
    ...
    public void setDataprov(ListProvider dataprov) {
        m_bd.setDataprov(dataprov);
    }
    public ListProvider getDataprov(){
        return m_bd.getDataprov();
    }
