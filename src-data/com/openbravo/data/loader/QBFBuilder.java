...
//    }
    ...
    public String getSQL(SerializerWrite sw, Object params) throws BasicException {
        ...
        if (sw == null || params == null) {
            ...
            if (sFilter.length() == 0) { 
                strReturn = m_sSentNullFilter;
                strReturn = strReturn.replaceAll("\\?\\(QBF_FILTER\\)", "(1 = 1)");
                return strReturn; // no hay filtro
            } else {
                strReturn =m_sSentBeginPart + "(" + sFilter + ")" + m_sSentEndPart;// incluimos el filtro
                strReturn = strReturn.replaceAll("\\?\\(QBF_FILTER\\)", "(" + sFilter + ")");
                return  strReturn;
            }
        }          
    }  
